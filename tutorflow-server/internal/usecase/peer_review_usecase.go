...
func (uc *peerReviewUseCase) ConfigurePeerReview(
	...
	existing, _ := uc.peerReviewRepo.GetConfigByLessonID(lessonID)
	if existing != nil {
		...
		return uc.peerReviewRepo.UpdateConfig(existing)
	}

	// Create new config
	config.LessonID = lessonID
	return uc.peerReviewRepo.CreateConfig(config)
}

// GetPeerReviewConfig returns peer review configuration for a lesson
func (uc *peerReviewUseCase) GetPeerReviewConfig(lessonID uuid.UUID) (*domain.PeerReviewConfig, error) {
	return uc.peerReviewRepo.GetConfigByLessonID(lessonID)
}
...
func (uc *peerReviewUseCase) AddCriteria(
	...
	return uc.peerReviewRepo.CreateCriteria(criteria)
}

// UpdateCriteria updates a review criteria
func (uc *peerReviewUseCase) UpdateCriteria(criteria *domain.PeerReviewCriteria) error {
	return uc.peerReviewRepo.UpdateCriteria(criteria)
}

// RemoveCriteria removes a review criteria
func (uc *peerReviewUseCase) RemoveCriteria(id uuid.UUID) error {
	return uc.peerReviewRepo.DeleteCriteria(id)
}

// AssignReviewers assigns reviewers to a submission
func (uc *peerReviewUseCase) AssignReviewers(submissionID uuid.UUID) error {
	// Get submission to find lesson
	assignments, err := uc.peerReviewRepo.GetAssignmentsForSubmission(submissionID)
...
func (uc *peerReviewUseCase) GetMyReviewAssignments(userID uuid.UUID) ([]domain.PeerReviewAssignment, error) {
	return uc.peerReviewRepo.GetAssignmentsForReviewer(userID)
}
...
func (uc *peerReviewUseCase) SubmitReview(
	...
	// Get the assignment
	assignment, err := uc.peerReviewRepo.GetAssignmentByID(assignmentID)
	if err != nil {
	...
	if assignment.Submission != nil && assignment.Submission.AssignmentID != uuid.Nil {
		config, _ = uc.peerReviewRepo.GetConfigByLessonID(assignment.Submission.AssignmentID)
	}
	...
	if err := uc.peerReviewRepo.CreateReview(review); err != nil {
		return err
	}
	...
	for _, score := range scores {
		...
		uc.peerReviewRepo.CreateScore(&score)
	}
	...
	return uc.peerReviewRepo.UpdateAssignment(assignment)
}
...
func (uc *peerReviewUseCase) DisputeReview(reviewID uuid.UUID, reason string) error {
	review, err := uc.peerReviewRepo.GetReviewByAssignmentID(reviewID)
	if err != nil {
		return errors.New("review not found")
	}

	// Get the assignment and mark as disputed
	assignment, err := uc.peerReviewRepo.GetAssignmentByID(review.AssignmentID)
	if err != nil {
		return errors.New("assignment not found")
	}
...
func (uc *peerReviewUseCase) AutoAssignPendingReviews() error {
...
func (uc *peerReviewUseCase) CalculateFinalScore(submissionID uuid.UUID) (float64, error) {
	reviews, err := uc.peerReviewRepo.GetReviewsForSubmission(submissionID)
	if err != nil {
		return 0, err
	}
